{ config, pkgs, modulesPath, ... }:
    ...
    serviceConfig = {
      ...
      PrivateTmp=true;
      KillMode="mixed";
      ExecStart = ''@${pkgs.openvpn}/sbin/openvpn openvpn --daemon ovpn-pia --status /run/openvpn/%i.status 10 --cd /etc/openvpn --script-security 2 --config ${config.sops.templates.vpn.path} --writepid /run/openvpn/pia.pid'';
