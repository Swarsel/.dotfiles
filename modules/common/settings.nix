let
  ...
  mkNixPlugins = pkgs: lib: pkgs.nix-plugins.overrideAttrs (old: {
  ...
  mkExtraOptions = { self, pkgs, lib, minimal, config }:
    ''
  ...
  mkAdditionsOverlay = { self, config, lib, homeConfig }:
    (final: prev:
      let
...
{
  flake.modules = {
    nixos.settings = { self, lib, pkgs, config, outputs, inputs, minimal, globals, withHomeManager, confLib, ... }:
      let
    ...
    homeManager.settings = { self, outputs, lib, pkgs, config, globals, confLib, minimal, type, ... }:
      let
