  ...
  perSystem = { pkgs, lib, system, ... }:
    {
      _module.args.testsLib = rec {
        inputSources =
          ...
          in
          builtins.attrValues (go { } (builtins.attrValues (builtins.removeAttrs self.inputs [ "vbc-nix" "repoSecrets" ])));

        mkDemoTest = name: testArgs:
          if !(inputs.repoSecrets.isDemo or false) then
            ...
            pkgs.testers.runNixOSTest (lib.recursiveUpdate
              {
                inherit name;
        ...
        installerNode = { extraDependencies ? [ ], diskSize ? 32 * 1024 }: { lib, ... }: {
          imports = [
          ...
          boot.supportedFilesystems = lib.mkOverride 40 [ "btrfs" "vfat" "ext4" ];
        ...
        targetNode = { toplevel, memorySize ? 4096, cores ? 2, quiet ? true }: {
          virtualisation = {
        ...
        prepareCloneSourceScript = machine: ''
          with subtest("Prepare local clone source"):
              ...
              ${machine}.succeed("GIT_COMMITTER_DATE='@${toString (self.lastModified or 1)}' GIT_AUTHOR_DATE='@${toString (self.lastModified or 1)}' git -C /root/dotfiles-src -c commit.gpgsign=false -c user.email=demo@example.org -c user.name=demo commit -q -m demo")
        ...
        unlockBootScript = { machine, marker, title ? "Boot installed system, unlock disk and reach a shell" }: ''
          with subtest("${title}"):
              ${machine}.start()
              booted = False
              for _ in range(40):
        ...
        enablePasswordlessSudoScript = { machine, user ? "swarsel", password ? "setup" }: ''
          import time
        ...
        unlockGraphicalBootScript = { machine, title ? "Boot full system, unlock disk and reach graphical.target", passphrase ? "demopass" }: ''
          import time
        ...
        consoleMarkerLoopScript = { machine, title, command, marker, tries ? 20, timeout ? 15, failure }: ''
          with subtest("${title}"):
              ok = False
